...
void fixDashboard(String path) {
  ...
  // Add deductionCtrl
  if (!c.contains('final deductionCtrl = TextEditingController(text: e.pendingPayDeduction.toString());')) {
    c = c.replaceFirst(
  ...
  final monthlySalaryPatternEdit = RegExp(r'controller: salaryCtrl,[\s\S]*?label: "Fixed Monthly Salary[^"]*",[\s\S]*?keyboardType: TextInputType.number,\n\s*\),\n\s*const SizedBox\(height: 10\),');
  ...
  if (addIndex != -1) {
    ...
    if (!sub.contains('pendingPayDeduction: double.tryParse(deductionCtrl.text)')) {
      final m = RegExp(r'perSkuRate: double.tryParse\(rateCtrl3.text\) \?\? 0.0,').firstMatch(sub);
      if (m != null) {
        c = c.replaceRange(addIndex + m.start, addIndex + m.end, 'perSkuRate: double.tryParse(rateCtrl3.text) ?? 0.0,\n                          pendingPayDeduction: double.tryParse(deductionCtrl.text) ?? 0.0,');
      }
    }
  }
