...
class _EcomLedgerDialogState extends State<EcomLedgerDialog> {
  ...
  void _showAddLogDialog() {
    ...
    showDialog(
      ...
      builder: (ctx) {
        return AlertDialog(
          title: const Text("Add SKU Log", style: TextStyle(fontWeight: FontWeight.bold)),
          content: Column(
  ...
  Widget build(BuildContext context) {
    ...
    final c = context.watch<AppState>().clients.firstWhere((cl) => cl.id == widget.client.id, orElse: () => widget.client);
    
    var logs = List.of(c.ecomSkuLogs)..sort((a, b) => b.timestamp.compareTo(a.timestamp));
    ...
    return AlertDialog(
      ...
      title: Row(
        ...
        children: [
          Text("Historical Ledger - ${c.name}", style: const TextStyle(fontWeight: FontWeight.bold)),
          ElevatedButton.icon(
            style: ElevatedButton.styleFrom(backgroundColor: SageColors.primary, foregroundColor: Colors.white),
