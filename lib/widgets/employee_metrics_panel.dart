...
class EmployeeMetricsPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    bool isSalary = employee.videoEditorPayType == 'Salary' || employee.monthlySalary > 0;
    
    // Roles
    bool roleGraphics = employee.hasRole('graphic');
    bool roleVideoEditor = employee.hasRole('video editor');
    bool roleVideographer = employee.hasRole('videographer') || employee.hasRole('videographer/cinematographer');
    ...
    return Column(
      ...
      children: [
        ...
        if (!roleGraphics && !roleVideoEditor && !roleVideographer) _buildLegacyMetrics(context),
      ],
    );
  }
  ...
  Widget _buildVideographerMetrics(BuildContext context) {
    // Pending sessions
    final pendingSessions = state.tasks.where((t) => 
      t.assignedTo == employee.id &&
      (t.taskType == 'Session' || t.taskType == 'Miscellaneous Session' || t.taskType == 'session' || t.taskType == 'miscellaneous session') &&
      t.isCompleted &&
    ...
    List<Widget> sessionWidgets = pendingSessions.map((t) {
      ...
      return Padding(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(client?.name ?? 'Unknown Client', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600), maxLines: 1, overflow: TextOverflow.ellipsis), Text(DateFormat('dd MMM yyyy').format(t.deadline), style: const TextStyle(fontSize: 9, color: Colors.black54))])),
            Text("₹${rate.toStringAsFixed(0)}", style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
          ],
        ),
      );
    }).toList();
    ...
    return ExpansionTile(
      ...
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            const Text("PENDING SESSIONS PAYMENTS", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.black54)),
            Text("₹${totalPayout.toStringAsFixed(0)}", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.red)),
          ],
        ),
        const SizedBox(height: 8),
        ...sessionWidgets,
