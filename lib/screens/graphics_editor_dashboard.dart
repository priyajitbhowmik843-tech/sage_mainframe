...
class GraphicsEditorDashboard extends StatefulWidget {
  ...
  @override
  State<GraphicsEditorDashboard> createState() =>
      _GraphicsEditorDashboardState();
}
...
class _GraphicsEditorDashboardState extends State<GraphicsEditorDashboard> {
  ...
  Widget _buildHomeTab(BuildContext context, AppState state, Persona persona) {
    ...
    final unpaidDesigns = completedDesigns
        .where((t) => !t.isPaidToGraphicsEditor)
        .toList();
      ...
      child: Row(
        children: [
          ...
          ElevatedButton(
            ...
            onPressed: () =>
                context.read<AppState>().approveGraphicsEditorSession(t.id),
  ...
  Widget _buildFinanceTab(
    ...
    final numPaid = completedDesigns
        .where((t) => t.isPaymentAcknowledgedByGraphicsEditor)
        .length;
    final numPending = completedDesigns.length - numPaid;

    final double amountPaid = emp.paidMonths.length * emp.monthlySalary;
    final double amountPending = emp.paymentCleared
        ? emp.pendingPayAmount
        : (emp.monthlySalary - deductionAmount);

    final designsPendingApproval = completedDesigns
        .where(
          (t) =>
              t.isPaidToGraphicsEditor &&
              !t.isPaymentAcknowledgedByGraphicsEditor,
        )
        .toList();
              ...
              Row(
                children: [
                  ...
                  Expanded(
                    child: StatChip(
                      label: 'DEDUCTIONS',
                      value: '\u20B9${deductionAmount.toStringAsFixed(0)}',
                ...
                children: [
                  ...
                  Expanded(
                    child: StatChip(
                      ...
                      value: '\u20B9${amountPending.toStringAsFixed(0)}',
                      valueColor: SageColors.error,
                      icon: Icons.pending,
                    ),
                  ),
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        ...
                        onPressed: () {
                          context
                              .read<AppState>()
                              .acknowledgeGraphicsEditorPayment(t.id);
                        },
  ...
  Widget _buildProfileTab(
    ...
    final displayPayout = emp.paymentCleared
        ? emp.pendingPayAmount
        : amountUnpaid;
    final displaySessions = emp.paymentCleared
        ? (emp.pendingPayMonth ?? "$numUnpaid")
        : "$numUnpaid";
        ...
        TerminalPanel(
          ...
          child: Column(
            children: [
              ...
              _profileRow(
                "PAID TILL MONTH",
                emp.paidMonths.isNotEmpty ? emp.paidMonths.last : 'None',
              ),
              _profileRow(
                "DEDUCTIONS",
                "\u20B9${deductionAmount.toStringAsFixed(0)}",
              ),
              const SizedBox(height: 16),
              SizedBox(
