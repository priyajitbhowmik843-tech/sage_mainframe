...
class _CeoDashboardState extends State<CeoDashboard> {
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  onPressed: () {
                    ...
                    context.read<AppState>().toggleClientPaidMonth(c.id, month, paymentMethod, paymentDate, amountOverride);
                    Navigator.pop(ctx);
                  },
  ...
  Widget _buildHomeTab() {
    ...
    final now = DateTime.now();
    final todayStart = DateTime(now.year, now.month, now.day);
    final pendingTasks = state.tasks.where((t) => !t.isCompleted && !(t.taskType ?? '').toLowerCase().contains('upload') && !t.title.toLowerCase().contains('upload')).length;
    ...
    int cfoCount = AppState.personas.where((p) => p.role == PersonaRole.ceo).length;
  ...
  Widget _buildPendingPaymentsList(AppState state) {
    ...
    final pendingEmployees = state.employees.where((e) {
      ...
      final isVideo = (e.hasRole('videographer') || e.hasRole('videographer/cinematographer')) && (e.videoEditorPayType != 'Salary' && e.monthlySalary == 0);
      final isVideoEditorPerVideo = e.hasRole('video editor') && (e.videoEditorPayType == 'Per Video Rate' && e.monthlySalary == 0);
      if (isVideo) {
        final unpaidSessionsCount = state.tasks.where((t) => t.assignedTo == e.id && t.taskType == 'Session' && t.isCompleted && !t.isPaidToVideographer).length;
        if (unpaidSessionsCount > 0) return true;
      }
      if (isVideoEditorPerVideo) {
        final unpaidVideosCount = state.tasks.where((t) => t.assignedTo == e.id && t.taskType != 'Session' && t.isCompleted && !t.isPaidToVideographer).length;
  ...
  Widget _buildClientsTab() {
    ...
    final reviewClients = state.clients.where((c) => !c.isApprovedByCeo || c.isTerminationRequested).toList();
    final displayedClients = _clientSubTab == 'ACTIVE' ? activeClients : (_clientSubTab == 'REVIEW' ? reviewClients : leadClients);
        ...
        Row(
          ...
          children: [
            Row(
              children: [
                Text(
                  _clientSubTab == 'ACTIVE' ? "ACTIVE CLIENT CONTRACTS" : (_clientSubTab == 'LEADS' ? "LEADS DATABASE" : (_clientSubTab == 'E-COM LEDGER' ? "E-COMMERCE SKU LEDGER" : "CLIENTS PENDING APPROVAL")),
                              ...
                              builder: (ctx) => AlertDialog(
                                ...
                                actions: [
                                  ...
                                  ElevatedButton(
                                    ...
                                    onPressed: () {
                                      Navigator.pop(ctx);
                                      context.read<AppState>().updateClientStatus(c.id, 'Active');
                                    },
      ...
      children: [
        SageCalendar(
          ...
          cellBuilder: (context, date) {
            ...
            final dayTasks = allTasks.where((t) => t.deadline.day == day && t.deadline.month == date.month && t.deadline.year == date.year).toList();
                        ...
                        if (_newTaskType == 'Session') ...[
                          ...
                          if (_sessionVideographerId != null) ...[
                            Builder(builder: (ctx) {
                              ...
                              final existingSessionsOnDate = state.tasks.where((t) => t.taskType == 'Session' && t.assignedTo == _sessionVideographerId && t.deadline.toString().substring(0,10) == dateKey).length;
                              return Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
            ...
            child: ExpansionTile(
              ...
              leading: Theme(
                ...
                child: Checkbox(
                  ...
                  onChanged: DateTime.now().isBefore(DateTime(t.deadline.year, t.deadline.month, t.deadline.day)) ? null : (val) {
                    if (val == true) {
    ...
    return Column(
      children: pendingTasks.map((t) {
          ...
          final submissionDateStr = t.submittedAt != null 
            ...
            : "Unknown";
        final deadlineStr = "${t.deadline.day.toString().padLeft(2, '0')}/${t.deadline.month.toString().padLeft(2, '0')}";
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text("Deadline: $deadlineStr", style: const TextStyle(fontSize: 11, color: Colors.black87, fontWeight: FontWeight.bold)),
                  Text("Submitted: $submissionDateStr", style: const TextStyle(fontSize: 11, color: Colors.black54)),
                ],
              ),
              if (t.description.isNotEmpty) ...[
                const SizedBox(height: 4),
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                      decoration: BoxDecoration(
                        color: deficit > 0 ? Colors.red.withOpacity(0.1) : Colors.green.withOpacity(0.1),
                        border: Border.all(color: deficit > 0 ? Colors.red : Colors.green),
                        borderRadius: BorderRadius.circular(16),
                      ),
                      child: Text("Deficit: \u20B9${deficit.toStringAsFixed(0)}", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: deficit > 0 ? Colors.red : Colors.green)),
                    ),
                  ],
                ),
                const SizedBox(height: 16),
            ...
            child: TerminalPanel(
              ...
              child: Column(children: [
                ...
                Wrap(
                  spacing: 6,
                  runSpacing: -8,
                  children: ['Video Editor', 'Graphics Editor', 'Videographer', 'Marketing Executive', 'Page Management Executive', 'Ecom Executive'].map((role) {
                ...
                const SizedBox(height: 10),
                SageTextField(controller: rateCtrl3, label: "Per SKU Rate (₹)", keyboardType: TextInputType.number),
  ...
  void _showEditClientDialog(BuildContext context, Client c) {
    ...
    showDialog(
      ...
      builder: (ctx) {
        return StatefulBuilder(
          builder: (ctx, setState) {            return AlertDialog(
              backgroundColor: SageColors.background,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (c.status == 'Lead') ...[
                      ...
                      const SizedBox(height: 10),
                      DropdownButtonFormField<String>(
                        value: context.read<AppState>().employees.any((e) => e.id == marketingExecutiveId && e.hasRole('marketing')) ? marketingExecutiveId : null,
                        ...
                        items: [
                          ...
                          ...context.read<AppState>().employees.where((e) => e.hasRole('marketing')).map(
