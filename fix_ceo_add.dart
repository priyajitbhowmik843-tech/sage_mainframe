...
void fixFile(String path) {
  ...
  final addMemberPattern = RegExp(r'void _showAddMemberDialog[\s\S]*?final rateCtrl3 = TextEditingController\(text: "0"\);');
  final match = addMemberPattern.firstMatch(c);
  if (match != null) {
    c = c.replaceRange(match.start, match.end, '${match.group(0)}\n    final deductionCtrl = TextEditingController(text: "0");');
  }
  ...
  final textPattern = RegExp(r'controller: rateCtrl3,[\s\S]*?label: "Per SKU Rate \(\\u20B9\)",[\s\S]*?keyboardType: TextInputType.number,\n\s*\),\n\s*const SizedBox\(height: 10\),');
  ...
  if (addMemberIndex != -1) {
    ...
    if (textMatch != null) {
      ...
      c = c.replaceRange(addMemberIndex + textMatch.end, addMemberIndex + textMatch.end, insertText);
    }
  }
