...
void main() {
  ...
  for (final path in files) {
    ...
    final notificationBugPattern = RegExp(
      r'"\$dateStr \$timeStr[^"]+by \$\{n\.triggeredBy\}"',
    );
    content = content.replaceAll(
      notificationBugPattern,
    ...
    final newPaidTill =
        r'''Text("Paid Till: ${employee.paidMonths.isEmpty ? 'None' : ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'].lastWhere((m) => employee.paidMonths.contains(m), orElse: () => employee.paidMonths.last)}"''';
