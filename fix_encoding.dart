...
void main() {
  ...
  for (final file in files) {
    ...
    content = content.replaceAll(RegExp(r'Amount \([^)]+\)'), r'Amount (\u20B9)');
    
    // Also fix the replaceAll logic in dart code
    content = content.replaceAll(r"replaceAll('A?sA1', '')", r"replaceAll('\u20B9', '')");
    content = content.replaceAll(RegExp(r"replaceAll\('A\xef\xbf\xbd\?sA1', ''\)"), r"replaceAll('\u20B9', '')");
    content = content.replaceAll(RegExp(r"replaceAll\('A\xef\xbf\xbd\?s', ''\)"), r"replaceAll('\u20B9', '')");
