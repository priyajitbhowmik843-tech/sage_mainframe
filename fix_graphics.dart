...
void main() {
  ...
  final homeMatch = RegExp(
    r'TerminalPanel\(\s*title: .UNPAID PAYMENT TRACKER.,[\s\S]*?\),\s*const SizedBox\(height: 14\),',
  ).firstMatch(c);
  if (homeMatch != null) {
  ...
  final financeStartMatch = RegExp(
    r'Widget _buildFinanceTab[\s\S]*?Widget _buildProfileTab',
  ).firstMatch(c);
  if (financeStartMatch != null) {
    ...
    c = c.replaceRange(
      financeStartMatch.start,
      financeStartMatch.end,
      financeBlock,
    );
  }
  ...
  final profileMatch = RegExp(
    r'Widget _buildProfileTab[\s\S]*?FINANCE DATA.,\s*child: Column\(\s*children: \[',
  ).firstMatch(c);
  if (profileMatch != null) {
    // we need to replace the profile rows inside FINANCE DATA
    final rowMatch = RegExp(
      r'_profileRow\("AMOUNT PAID"[\s\S]*?_profileRow\("UNPAID DESIGNS".*?\),',
    ).firstMatch(c);
    if (rowMatch != null) {
      String newRows = '''
