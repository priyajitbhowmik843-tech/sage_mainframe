...
void main() {
  void updateFile(String path) {
    ...
    String ceoMonthlyFeePattern = """                    c.ecomPaymentType == 'Per SKU'
    ...
    String newCeoMonthlyFee = """                    if (c.isWebsiteHandlingActive)
                      _buildClientDetailRow(
                        "Website Handling",
    ...
    if (content.contains(ceoMonthlyFeePattern)) {
      ...
      String cfoMonthlyFeePattern = """                    c.ecomPaymentType == 'Per SKU' ? _buildClientDetailRow("Base Monthly Fee", "?\${c.getPayableForMonth(DateTime.now().month, DateTime.now().year).toStringAsFixed(0)} (Actual varies by SKU logs)") : _buildClientDetailRow("Monthly Fee", "?\${c.monthlyPayable.toStringAsFixed(0)}"),""";
      
      String newCfoMonthlyFee = """                    if (c.isWebsiteHandlingActive) _buildClientDetailRow("Website Handling", "?\${c.websiteHandlingFee.toStringAsFixed(0)}"),
