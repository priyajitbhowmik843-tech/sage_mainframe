...
void main() {
  ...
  final badPendingCount =
      '''    final int pendingCount = state.tasks.where((t) {
      if (t.isCompleted) return false;
      final now = DateTime.now();''';
  final goodPendingCount =
      '''    final int pendingCount = state.tasks.where((t) {
      if (t.isCompleted) return false;
      if ((t.taskType ?? '').toLowerCase().contains('upload') || t.title.toLowerCase().contains('upload')) return false;
      final now = DateTime.now();''';
  if (ceoContent.contains(badPendingCount))
    ceoContent = ceoContent.replaceAll(badPendingCount, goodPendingCount);
  ...
  if (ceoContent.contains(badTopMetric))
    ceoContent = ceoContent.replaceAll(badTopMetric, goodTopMetric);

  // 3. Robustify pendingTasks filter in CEO
  final badPendingTasks =
      '''      if ((t.taskType ?? '').toLowerCase().contains('upload')) return false;''';
  final goodPendingTasks =
      '''      if ((t.taskType ?? '').toLowerCase().contains('upload') || t.title.toLowerCase().contains('upload')) return false;''';
  ...
  final badCfoPendingCount =
      '''    final int pendingCount = state.tasks.where((t) {
      if (t.isCompleted) return false;
      final isToday = t.deadline.year == now.year && t.deadline.month == now.month && t.deadline.day == now.day;''';
  final goodCfoPendingCount =
      '''    final int pendingCount = state.tasks.where((t) {
      if (t.isCompleted) return false;
      if ((t.taskType ?? '').toLowerCase().contains('upload') || t.title.toLowerCase().contains('upload')) return false;
      final isToday = t.deadline.year == now.year && t.deadline.month == now.month && t.deadline.day == now.day;''';
  if (cfoContent.contains(badCfoPendingCount))
    cfoContent = cfoContent.replaceAll(badCfoPendingCount, goodCfoPendingCount);
