...
const pattern = /Widget _buildGraphicsEditorMetrics\(BuildContext context\) \{[\s\S]*?(?=^\s*Widget _buildLegacyMetrics|\Z)/m;
const replacement = `Widget _buildGraphicsEditorMetrics(BuildContext context) {
...
if (newCode === code) {
    console.log('NO MATCH FOUND, trying alternate regex');
    const altPattern = /Widget _buildGraphicsEditorMetrics\(BuildContext context\) \{[\s\S]*?(?=^\s*Widget _build)/m;
    newCode = code.replace(altPattern, replacement);
    if (newCode === code) {
        console.log('STILL NO MATCH');
    } else {
        fs.writeFileSync('lib/widgets/employee_metrics_panel.dart', newCode, 'utf8');
